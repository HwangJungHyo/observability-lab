...
| 4 주문 API 가시화 | 주문이 느리거나 실패하는가? API·모의 결제·계측 | 요청량·지연·시스템 오류 구분 | 2~3 | 4-1~4-3 실습·원복 기록 완료; 4-4 현재 시각 UP 최종 확인 대기 |
| 5 로그 조사 | 실패한 요청에서 무슨 일이 있었나? Alloy·Loki | 요청 식별자로 오류 검색 | 2 | 예정 |
...
- [ ] 4-4: 현재 시각 두 대상 up=1 최종 확인 후 4장 종료
- [ ] 다음 챕터: 5장 Alloy·Loki 로그 수집

4-3a 결제 중단의 복구는 사용자 확인으로 완료했다.
4-3b는 평균 약 1.01~1.05초·오류율 0% 및 이후 평균 약 86~87ms를 관측했다.
최종 설정 0.08, 주문 201을 확인했다. 최종 UP 캡처는 과거 고정 시각이며 값 열이 잘려 있어 현재 상태 확인을 한 항목 남긴다.
