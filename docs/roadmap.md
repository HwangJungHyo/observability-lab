...
| 4 주문 API 가시화 | 주문이 느리거나 실패하는가? API·모의 결제·계측 | 요청량·지연·시스템 오류 구분 | 2~3 | 4-1·4-2 완료, 4-3 결제 중단·주문 복구 확인; 지표 정상화 확인 중 |
...
- [ ] 4-3a: 복구 후 두 대상 up=1, 오류율 0%, 지연 정상화 최종 확인
- [ ] 4-3b: 제어된 결제 지연 주입·관측·원복
- [ ] 4-4: 잔여 검증과 기록을 마친 뒤 4장 종료
- [ ] 다음 챕터: 5장 Alloy·Loki 로그 수집

기능 복구와 지표 정상화를 구분한다. 사용자는 현재 지표 하락을 관측 중이며, 완전 정상화는 아직 확인되지 않았다.
