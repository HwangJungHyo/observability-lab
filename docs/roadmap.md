...
실험 번호 001~007과 챕터 번호는 별개다. 실험 문서는 각 챕터의 증거로 연결한다.
...
| 4 주문 API 가시화 | 주문이 느리거나 실패하는가? API·모의 결제·계측 | 요청량·지연·시스템 오류 구분 | 2~3 | 4-1·4-2 완료, 4-3a 복구·지표 정상화 확인 완료, 4-3b 지연 실험 준비 |
...
- [ ] 4-3b: 제어된 결제 지연 주입·관측·원복
- [ ] 4-4: 잔여 검증과 기록을 마친 뒤 4장 종료
- [ ] 다음 챕터: 5장 Alloy·Loki 로그 수집

사용자가 두 대상 UP, 오류율 0%, 지연 안정화를 최종 확인했다. 정상화 수치와 정확한 시각은 별도 제공되지 않았다.
이번 결제 중단에서 지연도 발생했지만 제어된 지연 주입 실험을 완료한 것은 아니다.
